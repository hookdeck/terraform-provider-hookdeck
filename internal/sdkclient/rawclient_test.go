...
func TestRawClient_WithRateLimiter(t *testing.T) {
	...
	// Send 5 requests
	for i := 0; i < 5; i++ {
		_, err := rawClient.SendRequest(t.Context(), "GET", "/test", nil)
		if err != nil {
...
func TestRawClient_NoRateLimiter(t *testing.T) {
	...
	for i := 0; i < numRequests; i++ {
		...
		go func() {
			defer wg.Done()
			_, _ = rawClient.SendRequest(t.Context(), "GET", "/test", nil)
		}()
	}
...
func TestRawClient_RequestConstruction(t *testing.T) {
	...
	_, err := rawClient.SendRequest(t.Context(), "POST", "/test", opts)
	if err != nil {
